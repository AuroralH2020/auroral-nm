...
export const acceptContract: acceptContractController = async (req, res) => {
    ...
    const { decoded } = res.locals
    try {
        await ContractService.acceptContractRequest(ctid, decoded.org, decoded.sub, res.locals.audit)
        return responseBuilder(HttpStatusCode.OK, res, null)
    } catch (err) {
        const error = errorHandler(err)
        logger.error({ msg: error.message, id: res.locals.reqId })
...
export const rejectContract: rejectContractController = async (req, res) => {
    ...
    const { decoded } = res.locals
    try {
        await ContractService.rejectContractRequest(ctid, decoded.org, decoded.sub, res.locals.audit)
        return responseBuilder(HttpStatusCode.OK, res, null)
    } catch (err) {
        const error = errorHandler(err)
        logger.error({ msg: error.message, id: res.locals.reqId })
...
export const createContract: postContractController = async (req, res) => {
    ...
    const { decoded } = res.locals
    try {
        await ContractService.createOne(decoded.org, decoded.sub, termsAndConditions, organisations, res.locals.audit, description)
...
export const removeOrgFromContract: removeOrgFromContractCtrl = async (req, res) => {
    ...
    try {
        ...
        await ContractService.removeOrgFromContract(ctid,decoded.org,decoded.sub, res.locals.audit)
        return responseBuilder(HttpStatusCode.OK, res, null, null)
    } catch (err) {
        const error = errorHandler(err)
        logger.error({ msg: error.message, id: res.locals.reqId })
